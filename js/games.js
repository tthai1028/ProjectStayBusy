...
let jokeUrl = 'https://sv443.net/jokeapi/category/Miscellaneous?blacklistFlags=nsfw';
let jokeUrlNSFW = 'https://sv443.net/jokeapi/category/Miscellaneous';
// Fetch a joke from the API
function fetchJoke(url, callback) {
    // Make XMLHttpRequest
    let xhr = new XMLHttpRequest();
     xhr.setRequestHeader('Origin', 'https://tthai1028.github.io/ProjectStayBusy/')
    xhr.open('GET', url);
...
function updatePage(json) {
    if (json.joke) {
      jokeArea.innerHTML = json.joke;
      answerArea.innerHTML = "";
    } else {
      jokeArea.innerHTML = json.setup;
      answerArea.innerHTML = json.delivery;
    }
}
...
button.addEventListener('click', () => {
  if (checkbox.checked != true) {
    fetchJoke(jokeUrl, updatePage);
  } else {
    fetchJoke(jokeUrlNSFW, updatePage);
  }
});


// Selecting Element Reference
const sButton = document.getElementById("play");
